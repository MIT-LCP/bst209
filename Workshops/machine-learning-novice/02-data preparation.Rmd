...
set.seed(42)
train_index <- createDataPartition(y = cohort$actualhospitalmortality_enc, p = 0.7, list = FALSE)
x_train <- cohort[train_index, -which(names(cohort) == 'actualhospitalmortality')]
y_train <- cohort[train_index, 'actualhospitalmortality_enc']
x_test <- cohort[-train_index, -which(names(cohort) == 'actualhospitalmortality')]
y_test <- cohort[-train_index, 'actualhospitalmortality_enc']
```
...
scaler_fit <- predict(scaler, x_train)

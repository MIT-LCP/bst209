...
cohort$actualhospitalmortality_enc <- as.integer(cohort$actualhospitalmortality)
...
set.seed(42)
data_split <- createDataPartition(y = cohort$outcome, p = 0.7, list = FALSE)
x_train <- cohort[features, ][data_split, ]
y_train <- cohort[outcome][data_split]
x_test <- cohort[features, ][-data_split, ]
y_test <- cohort[outcome][-data_split]
...
for (i in 1:n_iterations) {
  ...
  y_bs <- y_train[index]
  # Make predictions
  y_hat <- predict(reg, newdata = data.frame(x_train = X_bs), type = "response")

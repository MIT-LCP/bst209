...
date: "2023-07-12"
questions:
- "What are limitations of traditional tabular data?"
- "How can they be mitigate through the event log format?"
...
So far, we have looked at data through a tabular lens: Every single data point represents one patient and all the information about the patient should be contained in that single data point. However, patient data is often composed of different treatments at different points in time. Not incorporating this leads to loss of contextual information and timing information.
...
The antibiotics doses in the previous table are aggregated amounts. We would like to express the individual administration of antibiotics. We add additional columns. When there is variation in the amounts of treatments, we have to including empty cells.
...
sepsis %>%
  check_rule(succession("IV Liquid","IV Antibiotics"), label = "r1") %>%
    group_by(r1) %>%
    n_cases()
```
